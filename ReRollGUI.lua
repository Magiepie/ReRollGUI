...
local NumHoradricSlots = 1

Horadric = CreateFrame("Frame", "Horadric", Horadric, "UIPanelDialogTemplate")
Horadric:SetSize(384, 512)--1 = 3x3,190= 3x3+1center, 230= 3x12, 
...
fontstring:SetText("NameHolder")

local ItemNamePlace = Horadric:CreateFontString(nil, "ARTWORK", "GameFontNormal")
ItemNamePlace:SetPoint("CENTER",0,145)
ItemNamePlace:SetText("")

local moneyplaceholder = 0
local moneyText = Horadric:CreateFontString(nil, "ARTWORK", "GameFontNormal")
moneyText:SetPoint("CENTER",0,75)
moneyText:SetText("Cost: "..GetCoinTextureString(moneyplaceholder))


local HoradricCombine = CreateFrame("BUTTON", "HoradricCombine", Horadric, "GameMenuButtonTemplate")
HoradricCombine:SetSize(120, 25)
HoradricCombine:SetText("Re-Roll");
HoradricCombine:SetScript("OnMouseDown", function(self, button) HoradricCombine_OnClick(self, button) end)
HoradricCombine:SetPoint("CENTER", Horadric, "BOTTOM", 0,295);

local button, lastButton, texture;
button = CreateFrame("BUTTON", "HoradricButton1", Horadric, "ItemButtonTemplate");
button:SetPoint("CENTER", 0, 110); --15 -30
...
function OnServerQueryReply(message)
	moneyText:SetText("Cost: "..GetCoinTextureString(message))
end
RegisterOpcodeHandler("RERR",OnServerQueryReply)

function HoradricPlaceItem(self)
	local infoType, info1, info2 = GetCursorInfo()
	local itemName, link, _, _, _, _, _,_, _, textureName,sellPrice = GetItemInfo(info1);			
...
StaticPopupDialogs["HORADRIC_ACCEPT"] = {
  text = "Are you sure you wish to Re-Roll this item?",
  button1 = "Yes",
  button2 = "No",
  OnAccept = function()
      HoradricCombineAccept()
